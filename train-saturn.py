...
class GeneEmbeddingMLP(nn.Module):
    def __init__(self, in_dim, out_dim=1280, hidden_dim=2048):
        super().__init__()
...
def trainer(args):
    ...
    #     # =====================================================
    #     # ⭐ 用 MLP 将 gene embedding 映射到 1280 维
    #     # =====================================================
    #     emb = species_gene_embeddings[species]   # Tensor [num_genes, D]
    #     in_dim = emb.shape[1]

    #     # 只在第一次时初始化 MLP
    #     if gene_mlp is None:
    #         gene_mlp = GeneEmbeddingMLP(
    #             in_dim=in_dim,
    #             out_dim=1280,
    #             hidden_dim=2048
    #         ).to(emb.device)
    #         gene_mlp.eval()          # 冻结
    #         for p in gene_mlp.parameters():
    #             p.requires_grad = False

    #     with torch.no_grad():
    #         species_gene_embeddings[species] = gene_mlp(emb)

    #     species_to_gene_embeddings.update(species_gene_embeddings)
    #     species_to_adata[species] = adata
        
    #     # print(species, species_to_gene_embeddings[species].shape)
    #     print(
    #         species,
    #         "original dim:", in_dim,
    #         "after MLP:", species_gene_embeddings[species].shape
    #     )
    ...
    for species, adata in species_to_adata.items():
        ...
        # =====================================================
        # ✅ 直接使用原始 gene embedding（不使用 MLP）
        # =====================================================
        emb = species_gene_embeddings[species]   # Tensor [num_genes, D]

        # 保持原始维度，不做任何映射或压缩
        species_gene_embeddings[species] = emb
        ...
        print(
            species,
            "gene embedding dim:", emb.shape[1]
        )
    ...
    print(f"Pretrain AnnData saved to: {pretrain_adata_path}")
    print("Pretraining finished. Skip metric learning.")

    return   # ⭐⭐⭐ 直接在这里结束 trainer
